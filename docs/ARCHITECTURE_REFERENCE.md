...
| requirements.txt | OK | Only non–pre-installed (pydantic-settings, sqlmodel, psycopg); psycopg no `[binary]` |
...
**Pre-installed:** databricks-sdk, fastapi, uvicorn, etc. — do not add to requirements.txt. **requirements.txt:** pydantic-settings, sqlmodel, psycopg (pure Python). **Compatibility:** Python 3.11, Node 22.16, Ubuntu 22.04. TanStack **1.158.1** with overrides; use `bun.lock`. See [Deployment guide](DEPLOYMENT_GUIDE.md#troubleshooting).
