...
This document confirms where the app gets its **data** and **visualizations** from. For how the UI connects to Lakebase (CRUD), how dashboards are embedded and where they get data, and how jobs/pipelines are triggered, see **[UI_BACKEND_DATABRICKS_WIRING.md](UI_BACKEND_DATABRICKS_WIRING.md)** (Cookbook / apx / AI Dev Kit alignment). When the Databricks Lakehouse is configured and available, **analytics and reporting data** are fetched from **Databricks** (Unity Catalog views, SQL Warehouse). **Embedded dashboards** are always Databricks Lakeview (DBSQL) dashboards deployed by the bundle. The UI is wired to the backend API, which in turn uses the Databricks SDK and SQL Warehouse for all Databricks resources (see [Apps Cookbook](https://apps-cookbook.dev/docs/intro), [apx](https://github.com/databricks-solutions/apx), [ai-dev-kit](https://github.com/databricks-solutions/ai-dev-kit)).
