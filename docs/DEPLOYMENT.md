...
**App resources in the bundle** (`resources/fastapi_app.yml`): SQL warehouse, UC volume (reports), Jobs 1–7, and four model serving endpoints. **Secret** and **Genie** are commented out so deploy succeeds; create secret scope `payment-analysis` and key `api-key` in **Workspace → Secrets** (and/or set `--var genie_space_id=<uuid>`), then uncomment the corresponding blocks in `resources/fastapi_app.yml` and redeploy to show them in the portal. For **full configuration**, add in **Compute → Apps → payment-analysis → Edit → Configure → App resources → + Add resource** after deploy: **Database** (Lakebase, Can connect and create), **MLflow experiment** (Can read), **Vector search index** (Can select), **UC function** (Can execute), **UC connection** (Use connection).
