...
**App resources in the bundle** (`resources/fastapi_app.yml`): SQL warehouse, Secret (scope `payment-analysis`, key `api-key`), Genie space (set `--var genie_space_id=<uuid>`), UC volume (reports), Jobs 1–7, and four model serving endpoints. For **full configuration** (all resource types from the Apps UI “Add resource” list), add the following in **Workspace → Compute → Apps → payment-analysis → Edit → Configure → App resources → + Add resource** after first deploy (bundle YAML does not yet support these): **Database** (Lakebase instance + database, Can connect and create), **MLflow experiment** (Can read), **Vector search index** (`<catalog>.<schema>.similar_transactions_index`, Can select), **UC function** (e.g. agent tool functions, Can execute), **UC connection** (Use connection).
