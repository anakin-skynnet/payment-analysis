...
- **App and user authorization (service principal + user token):** Use the **service principal** (DATABRICKS_CLIENT_ID / DATABRICKS_CLIENT_SECRET) for app-level operations (e.g. app_config, session, stats); use the **user token** (X-Forwarded-Access-Token) for user-scoped operations (warehouse queries, run job). Dependencies: `get_workspace_client_app_identity` and `get_databricks_service_app_identity` for SP-only; `get_workspace_client` / `get_databricks_service` use user token when present, then PAT or SP. **Getting the user token:** See [APP_AUTH_HEADERS.md](APP_AUTH_HEADERS.md) for how to read X-Forwarded-Access-Token in FastAPI, Flask, Gradio, Shiny, and Streamlit.
