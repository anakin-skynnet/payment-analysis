...
See docs/REFERENCE_GUIDE.md. Set DATABRICKS_CATALOG / DATABRICKS_SCHEMA if different.
...
def main() -> int:
    ...
    if args.command == "status":
        ...
        if failed_terminal and once:
            print("\nHint: If failures mention catalog/schema not found, see docs/REFERENCE_GUIDE.md#fix-catalog-or-schema-not-found", file=sys.stderr)
        ...
        try:
            for key in failed:
                ...
                if "catalog" in msg.lower() and ("not found" in msg.lower() or "cannot be found" in msg.lower()):
                    print(
                        "\nHint: Catalog/schema not found. Create the Unity Catalog (Data → Catalogs) and redeploy the bundle so the schema exists. See docs/REFERENCE_GUIDE.md#fix-catalog-or-schema-not-found",
                        file=sys.stderr,
                    )
                    break
        except Exception:
